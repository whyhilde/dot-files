...
BASE_PACKAGES = [ "xmonad", "xmonad-contrib", "ghc", "rofi", "dunst", "git", "firefox", "telegram-desktop", "obsidian", "bitwarden", "blender", "thunderbird", "neovim", "steam", "obs-studio", "libreoffice-fresh", "libreoffice-fresh-ru", "ghostty", "fastfetch", "cmus", "btop", "cava", "opendoas", "nemo", "feh", "pavucontrol", "flameshot", "gpick", "networkmanager", "network-manager-applet", "bluez", "blueman", "curl", "wget", "gzip", "unzip", "unrar", "xorg-xrandr", "xorg-xset", "xorg-setxkbmap", "xorg-xev", "xorg-xprop", "xorg-xmessage", ]
...
def install_programs():
    drivers_input = (
        input("Установить драйвера для Intel & NVIDIA? [Y/n]: ").strip().upper()
    )
...
def change_shell():
    user_input = input("Сменить shell на zsh? [Y/n]: ").strip().lower()
    ...
    if user_input == "Y" or user_input == "":
        ...
        while True:
            ...
            if theme_color not in colors:
                print(
                    f"{Cols.WARN}Ошибка: цвет {theme_color} не поддерживается!{Cols.END}"
                )
            else:
                break
